...
fetch(`http://localhost:3000/api/teddies/${productId}`)
    ...
    .then(teddy => {
        ...
        teddyProduct.appendChild(teddyColorSelect);  // adding the image inside the <article> element
        ...
        */
        ...
        buttonToCart.addEventListener("click", (event) => {  // configuring the events when the button is clicked
            ...
            const sendingProductInformations = {  // defining an object with the informations to send to cart
                ...
                sentProductQuantity: 1 // sentProductQuantity: selectQuantityValue  // sending the selected quantity
            };
            ...
            if (productsInStorage) {  // defining what to do when there is existing data in LocalStorage
                console.log("Products in storage", productsInStorage)
                // if (productsInStorage.sentProductId === teddy._id) { // works only when existing data = replace existing values
                // productsInStorage.pop();  // delete last added informations (test)
